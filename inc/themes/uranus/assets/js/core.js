...
uranus.signup = function()
{
    ...
    $form.find(".dot").eq(0).on("click", function() {
        ...
        var $label = $form.find(".formnav .label");
        $label.text($label.data("text").replace("{step}", 1));
    });
        ...
        $.ajax({
            ...
            success: function(resp) {
                ...
                if (resp.result == 1) {
                    ...
                    var $label = $form.find(".formnav .label");
                    $label.text($label.data("text").replace("{step}", 2));
                } else {   
                    $form.find(".js-emailcheck>div").html(resp.msg);
                    $form.find(".js-emailcheck").removeClass('none');
                }
            }
        });
    });
